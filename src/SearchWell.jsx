  ...
  const performSearch = async (searchTerm) => {
    ...
    try {
      if (isReference) {
        ...
        // 🧹 CLEAN & SORT: Remove junk, sort by Bible Order (1-66)
        const rawResults = Object.values(data)
            .filter(item => item.book && BOOK_ID_MAP[item.book]) // Remove items with no valid book ID
            .sort((a, b) => {
                // Strict Sort: Book First, then Chapter, then Verse
