...
function BibleTracker({ readChapters = [], onNavigate, sectionFilter = null }) {
  ...
  if (selectedBook) {
    ...
    return (
      <div style={{ padding: '20px', backgroundColor: 'white', borderRadius: '15px', border: '1px solid #eee', textAlign: 'center', marginBottom: '20px' }}>
  ...
  const renderSection = (sectionCode, title) => {
    ...
    return (
      <div style={{ marginBottom: '20px' }}>
        ...
        {!sectionFilter && (
            <h4 style={{ color: '#555', borderBottom: '2px solid #eee', paddingBottom: '10px', marginBottom: '15px' }}>
            {title}
            </h4>
        )}
  ...
  return (
    <div style={{ marginTop: '20px', padding: '10px', backgroundColor: 'white', borderRadius: '15px', border: sectionFilter ? 'none' : '1px solid #eee' }}>
      
      {/* Show Global Stats ONLY if we are in the bottom "full" mode (no filter) */}
      {!sectionFilter && (
        <div style={{ textAlign: 'center', marginBottom: '30px' }}>
            <h3 style={{ margin: 0, color: '#333' }}>Your Living Bookshelf</h3>
            <p style={{ color: '#777', fontSize: '0.9rem', marginTop: '5px' }}>
            Total Progress: <strong>{totalPercentage}%</strong>
            </p>
            <div style={{ width: '100%', height: '10px', background: '#f0f0f0', borderRadius: '5px', marginTop: '10px', overflow: 'hidden' }}>
